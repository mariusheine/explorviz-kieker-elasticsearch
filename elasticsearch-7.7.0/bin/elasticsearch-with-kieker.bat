...
ECHO.!KEYSTORE_PASSWORD!| %JAVA% %ES_JAVA_OPTS% -Delasticsearch ^
  ...
  -Djava.security.policy=..\kieker-config\security.policy ^
  -javaagent:..\kieker-config\kieker-1.14-aspectj.jar ^
  -Dkieker.monitoring.skipDefaultAOPConfiguration=true ^
  -Dkieker.monitoring.configuration=..\..\kieker-config\META-INF\kieker.monitoring.properties ^
  -Dorg.aspectj.weaver.loadtime.configuration=..\..\kieker-config\META-INF\aop.xml ^
